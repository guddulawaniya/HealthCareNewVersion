...
class AdapterPlannerDate(
    private val context : Context,
    private val datalist: List<date_response_task>,
    private var selectedPosition: Int = -1,
    private val itemClickListener: (date_response_task, Int) -> Unit,
    ) : RecyclerView.Adapter<AdapterPlannerDate.ViewHolderClass>()  {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderClass {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.planner_date_layout, parent, false)
...
//        holder.whole.setBackgroundResource(R.drawable.date_time_selector_background)
        if (isSelected) {
            ...
            holder.whole.setBackgroundResource(R.drawable.datechooseplannerselect_background)
        }else
        {
            val textColor = ContextCompat.getColorStateList(context, R.color.CC6FF8)
            holder.day.setTextColor(textColor)
            holder.date.setTextColor(textColor)
            holder.whole.setBackgroundResource(R.drawable.datechoose_planner_background)
        }
