...
import com.asyscraft.alzimer_module.databinding.ActivityMatchingChalelnge2Binding
...
@AndroidEntryPoint
class MatchingChallenge2 : BaseActivity() {
    ...
    private lateinit var binding: ActivityMatchingChalelnge2Binding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityMatchingChalelnge2Binding.inflate(layoutInflater)
        ...
        binding.progressView.post {
            ...
            val clipDrawable = drawable.findDrawableByLayerId(R.id.fill) as ClipDrawable

            val filledParts = 7 // or 2, 3... up to 16
        ...
        binding.next.setOnClickListener {
            ...
            val answer = AnswerRequest(
                question = "6814bdc0aea0c2912b180b27",
                isCorrect = true,
                points = point,
                isSkipped = false
            )
            ...
            val intent = Intent(this, Rotate_game::class.java)
    ...
    private fun observeDataQuestion() {
        lifecycleScope.launch {
            lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.dataQuestion.observe(this@MatchingChallenge2) { response ->
                    when (response) {
                        is Resource.Success -> {
                            Progresss.stop()
                                ...
                                if (questionResponse.success) {
                                    ...
                                    question?.let { q ->
                                        ...
                                        if (imageList.size >= 4) {
                                            ...
                                            for (i in 0 until 8) {
                                                Picasso.get()
                                                    .load(Constants.IMAGE_BASEURL + mutableDuplicatedImages[i])
                                                    .into(imageViews[i])
                                            }

                                        } else {
                                            androidExtension.alertBox(
                                                "Not enough images to load",
                                                this@MatchingChallenge2
                                            )
                        ...
                        is Resource.Loading -> {
                            Progresss.start(this@MatchingChallenge2)
